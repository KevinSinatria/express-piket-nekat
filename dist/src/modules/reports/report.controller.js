...
const getStudentPermits = async (req, res, next) => {
    try {
        const data = await reportService.getStudentPermitReportData(prisma, req.query);
        res.status(200).json({
            success: true,
...
const exportStudentPermits = async (req, res, next) => {
    try {
        const data = await reportService.getStudentPermitReportData(prisma, req.query);
        ...
        res.setHeader("Content-Disposition", `attachment; filename=student-permit-report-${Date.now()}.xlsx`);
